...
let speechQfile = JSON.parse(
  fs.readFileSync("./text/speechquery.json", "utf-8")
);
...
function enteredChannel(username) {
  ...
  if (vConnection) {
    googleTTS(
      username + msgs.enteredchannel,
      speechlang,
      cfg.consts.speechspeed
    )
      .then(function (url) {
        if (ytdispatcher) ytdispatcher.destroy();
...
client.on("message", (msg) => {
  if (msg.channel instanceof Discord.TextChannel) {
    ...
    if (msg.content[0] == cfg.prefix) {
      ...
      switch (cmd) {
                    ...
                    if (speaking.bitfield == 1) {
                      ...
                      const recognizeStream = speechclient
                        ...
                        .on("data", (response) => {
                          ...
                          var botresponse = botspeech.botSpeechResponse(
                            (text = transcription),
                            (vc = vc),
                            (botnick = cfg.botnick)
                          );
        ...
        case "play":
          if (!rest[0]) {
            return msg.reply(
              " what to play ? - !play {youtube-link} - !play {keywords}"
            );
          }
          var vc = msg.member.voice.channel;
          if (!vc) {
        ...
        case "vol":
          ...
          if (ytdispatcher) ytdispatcher.setVolume(_musicVolume);
            ...
            async function (err, results) {
              ...
              for (let index = 0; index < results.length; index++) {
                ...
                if (img) {
                  ...
                  if (lx == undefined) {
                    return msg.reply(
                      " beklenmedik bi sorun oldu. beklenseydi zaten cozmus olurduk"
                    );
                  }
    ...
    // if the msg sent via private msg and, msg author is the admin
    if (msg.author.id == cfg.discord.admin) {
...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (
    newState.channelID &&
    newState.channelID == cfg.discord.channels.voice &&
    ...
    oldState.channelID &&
    oldState.channelID == cfg.discord.channels.voice &&
    newState.channelID != oldState.channelID
  ) {
